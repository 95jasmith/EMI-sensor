...
char rioOutput;
char commport[] = "\\\\.\\COM";
INT32 rioInputPort = 13;
char tempport[] = "00";
char* rioNamePort = commport;
...
void InitOpt(AnyOption& opt)
{
	...
	opt.addUsage(" -r  --riocom 12      rio Port ");
	opt.addUsage(" -p  --filepath <File_Name>     File path for logging");
	opt.addUsage("");
	...
	opt.setOption("riocom", 'r'); //myrio com Port
	opt.setOption("filepath", 'p'); //File path for logging
...
int main(int argc, char* argv[])
{
	...
	if (opt.getValue('c') != NULL || opt.getValue("gemcom") != NULL)
	{
		/* COM port or device */
		if (opt.getValue('c'))
			error = sscanf(opt.getValue('c'), "%u", &COMport);
		else
			error = sscanf(opt.getValue("gemcom"), "%u", &COMport);

	}
	if (opt.getValue('b') != NULL || opt.getValue("gembaud") != NULL)
	{
		/* baud */
		if (opt.getValue('b'))
			error = sscanf(opt.getValue('b'), "%u", &COMbaud);
		else
			error = sscanf(opt.getValue("gembaud"), "%u", &COMbaud);
	}

	if (opt.getValue('r') != NULL || opt.getValue("riocom") != NULL)
	{
		// baud 
		if (opt.getValue('r'))
			error = sscanf(opt.getValue('r'), "%u", &rioInputPort);
		else
			error = sscanf(opt.getValue("riocom"), "%u", &rioInputPort);
	}
	if (opt.getValue('f') != NULL || opt.getValue("filepath") != NULL)
	{
		// baud 
		if (opt.getValue('f'))
			error = sscanf(opt.getValue('f'), "%s", &outFilePath);
		else
			error = sscanf(opt.getValue("filepath"), "%s", &outFilePath);
	}
	...
	// convert Port input to string and add to commport, default if 12
	sprintf(tempport, "%d", rioInputPort);
	strcat(commport,tempport);

	SerialPort rioPort(rioNamePort);
	if (rioPort.isConnected()) {
		cout << "Connection is Established\n";
	}
	else {
		cout << "Error in myRio port name";
	}
	...
	do {
		...
		if(_kbhit() || rioPort.readSerialPort(&rioOutput, 1)) {
			if (rioOutput) {
				KbChar = rioOutput;
				rioOutput = 0;
			}
			...
			switch(KbChar)
			{
				...
				case 'L':  
					...
					if (!bLogging)
					{
						...
						cout << outFilePath;
						if (strcmp("0", outFilePath) != 0){
							strcpy_s(retname, MAX_PATH, outFilePath);
							strcat(retname, "gemdata");
						}
						else {
