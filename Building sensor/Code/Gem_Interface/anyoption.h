...
public: /* the public interface */
	AnyOption();
	AnyOption(long maxoptions ); 
	AnyOption(long maxoptions , long maxcharoptions); 
	~AnyOption();
	...
	void useCommandArgs( long _argc, char **_argv );
	...
	void processCommandArgs( long max_args );
	bool processFile();

	/*
         * process the specified options 
         */
	void processCommandArgs( long _argc, char **_argv );
	void processCommandArgs( long _argc, char **_argv, long max_args );
	...
	/* 
         ...
         */
	long   getArgc();
	char* getArgv( long index );
	bool  hasOptions();

private: /* the hidden data structure */
	long argc;		/* commandline arg count  */
	char **argv;  		/* commndline args */
	const char* filename; 	/* the option file */
	char* appname; 	/* the application name from argv[0] */

	long *new_argv; 		/* arguments sans options (index to argv) */
	long new_argc;   	/* argument count sans the options */
	long max_legal_args; 	/* ignore extra arguments */


	/* option strings storage + indexing */
	long max_options; 	/* maximum number of options */
	const char **options; 	/* storage */
	long *optiontype; 	/* type - common, command, file */
	long *optionindex;	/* index into value storage */
	long option_counter; 	/* counter for added options  */

	/* option chars storage + indexing */
	long max_char_options; 	/* maximum number options */
	char *optionchars; 	/*  storage */
	long *optchartype; 	/* type - common, command, file */
	long *optcharindex; 	/* index into value storage */
	long optchar_counter; 	/* counter for added options  */

	/* values */
	char **values; 		/* common value storage */
	long g_value_counter; 	/* globally updated value index LAME! */

	/* help and usage */
	const char **usage; 	/* usage */
	long max_usage_lines;	/* max usage lines reseverd */
	long usage_lines;	/* number of usage lines */
...
private: /* the hidden utils */
	void init();	
	void init(long maxopt, long maxcharopt );	
	bool alloc();
	void cleanup();
	bool valueStoreOK();
	...
	void addOption( const char* option , long type );
	void addOption( char optchar , long type );
	...
	long parseGNU( char *arg );
	bool matchChar( char c );
	long matchOpt( char *opt );

	/* dot file methods */
	char *readFile();
	char *readFile( const char* fname );
	bool consumeFile( char *buffer );
	void processLine( char *theline, long length );
	char *chomp( char *str );
