...
AnyOption::AnyOption(long maxopt)
{
	init( maxopt , maxopt );
}

AnyOption::AnyOption(long maxopt, long maxcharopt)
{
...
void
AnyOption::init(long maxopt, long maxcharopt )
{
...
AnyOption::alloc()
{
	...
	optiontype = (long*) malloc( (max_options+1)*sizeof(long) );	
	optionindex = (long*) malloc( (max_options+1)*sizeof(long) );	
	...
	optchartype = (long*) malloc( (max_char_options+1)*sizeof(long) );	
	optcharindex = (long*) malloc( (max_char_options+1)*sizeof(long) );	
	if( optionchars == NULL || 
            optchartype == NULL || 
            optcharindex == NULL )
        {
...
AnyOption::doubleOptStorage()
{
	...
	optiontype = (long*) realloc(  optiontype ,  
			((2 * max_options)+1)* sizeof(long) );	
	optionindex = (long*) realloc(  optionindex,  
			((2 * max_options)+1) * sizeof(long) );	
	if( options == NULL || optiontype == NULL || optionindex == NULL )
		return false;
	/* init new storage */
	for( long i = max_options ; i < 2*max_options ; i++ ){
		options[i] = NULL;
		optiontype[i] = 0 ;
		optionindex[i] = -1 ;
	}
...
AnyOption::doubleCharStorage()
{
	...
	optchartype = (long*) realloc( optchartype,  
			((2*max_char_options)+1)*sizeof(long) );	
	optcharindex = (long*) realloc( optcharindex,  
			((2*max_char_options)+1)*sizeof(long) );	
	if( optionchars == NULL || 
	    optchartype == NULL || 
	    optcharindex == NULL )
		return false;
	/* init new storage */
	for( long i = max_char_options ; i < 2*max_char_options ; i++ ){
		optionchars[i] = '0';
		optchartype[i] = 0 ;
		optcharindex[i] = -1 ;
	}
...
AnyOption::doubleUsageStorage()
{
	...
	if ( usage == NULL )
		return false;
	for( long i = max_usage_lines ; i < 2*max_usage_lines ; i++ )
		usage[i] = NULL;
...
AnyOption::setCommandLongPrefix( char *_prefix )
{
	...
	strcpy_s (long_opt_prefix,3,  _prefix);
}
...
void
AnyOption::useCommandArgs( long _argc, char **_argv )
{
	argc = _argc;
	argv = _argv;
	command_set = true;
...
void
AnyOption::addOption( const char *opt, long type )
{
...
void
AnyOption::addOption( char opt, long type )
{
	if( !POSIX() ){
...
void 
AnyOption::processCommandArgs(long max_args)
{
...
void
AnyOption::processCommandArgs( long _argc, char **_argv, long max_args )
{
	max_legal_args = max_args;
...
void
AnyOption::processCommandArgs( long _argc, char **_argv )
{
...
AnyOption::processCommandArgs()
{
	...
	if( max_legal_args == 0 )
		max_legal_args = argc;
	new_argv = (long*) malloc( (max_legal_args+1) * sizeof(long) );
	for( long i = 1 ; i < argc ; i++ ){/* ignore first argv */
		if(  argv[i][0] == long_opt_prefix[0] && 
                     argv[i][1] == long_opt_prefix[1] ) { /* long GNU option */
			long match_at = parseGNU( argv[i]+2 ); /* skip -- */
			...
			if( POSIX() ){ 
				...
				long match_at = parseGNU( argv[i]+1 ); /* skip - */
				if( match_at >= 0 && i < argc-1 ) /* found match */
					setValue( options[match_at] , argv[++i] );
			}
...
{
	for( long i = 0 ; i < option_counter ; i++ ){
...
{
	for( long i = 0 ; i < optchar_counter ; i++ ){
...
AnyOption::valueStoreOK( )
{
	...
	if( !set ){
		if( g_value_counter > 0 ){
			...
			for( long i = 0 ; i < g_value_counter ; i++)
				values[i] = NULL;
			set = true;
		}
	}
...
void
AnyOption::processLine( char *theline, long length  )
{
        bool found = false;
        ...
        if( *cursor == delimiter || *(cursor+length-1) == delimiter ){
                ...
                for( long i = 1 ; i < length-1 && !found ; i++){/* delimiter */
                        if( *cursor == delimiter ){
